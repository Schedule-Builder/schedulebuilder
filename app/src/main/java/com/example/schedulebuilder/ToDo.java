...
public class ToDo extends Event  {
    ...
    /**public int[] makeTime(Day day){
        int[] times = new int[2];
        Event e;
        float efficiency = Integer.MAX_VALUE;
        float temp;
        int counter;
    ...
    public float calculateEfficiency(Day day, int startTime, int endTime){
        ...
        for (int i = 0; i<day.getByTime().size(); i++){
            ...
            if (e.getStartTime() >= startTime){
                efficiency += e.overlap(startTime,endTime)*(priority - e.getPriority());
    ...
    public Day freestTime(Day day, ArrayList<Integer> arr){
        ...
        for (int j = 0; j<arr.size()/2; j++){
            ...
            for (int i = 0; i < day.getByTime().size(); i++){
                ...
                if (num != 0)
                    temp.push(e);
                if (temp.getByTime().size()!=0 && num == 0)
                    break;
            }
    ...
    public float efficiencyBlock(Day days){
        ...
        if(days.getByTime().size()>=1) {
            if (Time.difference(days.getByTime().get(0).getStartTime(), days.getByTime().get(days.getByTime().size() - 1).getEndTime()) < temp)
                return efficiency;
        }
        efficiency = 0;
  ...
  /*public int[] findTimes(Day day){
    ...
    public void insertEvent(Day day, int startTime, int endTime){
        Event e = new Event(name, false, false, startTime, endTime,(int) priority);
        day.push(e);
        int index = day.findInTime(e);
        day.remove(day.getByTime().get(index+1));
